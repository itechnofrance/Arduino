//
// Page HTML pour gérer le mode suiveur de lumière
//

const char car2wd_suiveur_lumiere[] PROGMEM = R"=====(
<!DOCTYPE HTML>
  ...
  if(xmlHttp.readyState==0||xmlHttp.readyState==4)
  {
    xmlHttp.onreadystatechange=function()
    {
      if(xmlHttp.readyState==4&&xmlHttp.status==200)
      {
        ...
        xmlmsg=xmlDoc.getElementsByTagName('slidersensibilite')[0].firstChild.nodeValue;
        document.getElementById('slider_sensibilite').value=xmlmsg;
        document.getElementById('Slidersensibilite').innerHTML=xmlmsg;  
      }
    }
...
function slider_lux()
{
  Slidersensibilite=document.getElementById('slider_sensibilite').value;
  document.getElementById('Slidersensibilite').innerHTML=Slidersensibilite;
  if(xmlHttp.readyState==0||xmlHttp.readyState==4)
  {
    xmlHttp.open('PUT','set_sensibilite_lux?val='+Slidersensibilite,true);
    xmlHttp.send(null);
  }
}
</SCRIPT>
...
<BODY onload="process()">
<div align="Center">
  <h3>CAR-2WD mode suiveur de lumi&egravere</h3>
...
<div align="Center">
  Sensibilit&eacute : <A id=Slidersensibilite> </A> </br>
  <input style="width:250px; height:50px" type="range" id="slider_sensibilite" ONINPUT=slider_lux() min="0" max="500" step="10"/> </br>  
